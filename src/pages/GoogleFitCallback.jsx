...
export default function GoogleFitCallback() {
  ...
  async function exchangeCode(code) {
    try {
      setStatus('Se conectează Google Fit...')
      const redirectUri = import.meta.env.VITE_APP_URL + '/auth/googlefit/callback'
      const { data, error } = await supabase.functions.invoke('googlefit-auth', {
        body: { action: 'exchange', code, userId: user.id, redirectUri }
      })
      if (error) throw new Error(error.message)
      if (!data?.ok) throw new Error(data?.error || 'Eroare necunoscută')
      setStatus('✓ Google Fit conectat cu succes!')
      setDone(true)
